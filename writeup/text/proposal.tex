...
The storage space required for a server is therfore \todo{server storage}\\

Assuming each user opens there client just once every other day \todo{blah blah}
. To mitigate this we may run as a daemon on linux, or a background process in
windows, that starts when the user logs in \todo{analyse this method w/r/t how
ofter the computer is turned on}.
